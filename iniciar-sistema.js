...
class SistemaStarter {
  ...
  async iniciarComponente(tipo, directorio) {
    this.mensaje('INFO', this.colors.blue, `Iniciando ${tipo} en modo DESARROLLO...`);
    ...
    if (tipo === 'backend') {
      try {
        ...
        // Verificar conexión a PostgreSQL antes de ejecutar migraciones
        let postgresListo = false;
        for (let i = 0; i < 5; i++) {
          try {
            await this.ejecutarComando('docker', ['exec', 'hogar-ancianos-backend-db-1', 'pg_isready', '-U', 'hogar_user'], dirPath);
            postgresListo = true;
            this.mensaje('INFO', this.colors.green, 'PostgreSQL está listo');
            break;
          } catch (e) {
            this.mensaje('INFO', this.colors.yellow, `Esperando PostgreSQL (${i + 1}/5)...`);
            await new Promise(resolve => setTimeout(resolve, 1500));
          }
        }
        
        if (!postgresListo) {
          throw new Error('PostgreSQL no se pudo inicializar correctamente');
        }
    ...
    // Iniciar el componente en modo desarrollo
  ...
  async iniciar() {
    ...
    this.mensaje('INFO', this.colors.blue, 'Sistema listo para desarrollo - Hogar de Ancianos La Misericordia');
